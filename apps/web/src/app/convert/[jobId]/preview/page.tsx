      ...
      {column_status.suggestions.length > 0 && (
        <div className="bg-blue-50 border border-blue-200 rounded p-4 mb-6">
          ...
          {column_status.suggestions.map((s) => (
            <p key={`${s.csv_column}-${s.suggested_match}`} className="text-sm text-blue-600">
        ...
        <table className="w-full text-xs">
          ...
          <tbody>
            {preview.rows.map((row, rowIndex) => (
              <tr key={`row-${rowIndex}-${preview.headers.map(h => row[h] || '').join('-').slice(0, 50)}`} className="border-b hover:bg-gray-50">
